...
function renderAll() {
  ...
  const activeAccounts = new Set(state.accounts.map((account) => String(account.id)));
  const accountBalances = state.balances.filter((account) => activeAccounts.has(String(account.id)));
  const sumBalances = (predicate) => accountBalances.reduce((total, account) => total + (predicate(account) ? Number(account.balance) || 0 : 0), 0);
  const maddyMoney = sumBalances((account) => account.holder_name === 'Maddy' && account.owner_name !== 'Bryan');
  const bryanMoneyHeldByMaddy = sumBalances((account) => account.owner_name === 'Bryan' && account.holder_name === 'Maddy');
  const bryanMoneyHeldByBryan = sumBalances((account) => account.owner_name === 'Bryan' && account.holder_name === 'Bryan');
  ...
  $('#maddy-held').textContent = formatBalance(maddyMoney);
  $('#bryan-held-maddy').textContent = formatBalance(bryanMoneyHeldByMaddy);
  $('#bryan-held-bryan').textContent = formatBalance(bryanMoneyHeldByBryan);
