-- Dana Bryan dibagi menurut siapa yang memegangnya, tanpa mengubah pemiliknya:
-- Rp3.800.000 tetap pada "Uang Bryan" (dipegang Maddy) dan Rp200.000
-- dipindahkan ke rekening yang dipegang langsung oleh Bryan.
--
-- Pada database baru yang belum memiliki saldo, hanya rekeningnya yang dibuat.
-- Perpindahan Rp200.000 hanya dibuat jika saldo "Uang Bryan" mencukupi.
...
select
  ...
  held_by_maddy.id,
  held_by_bryan.id,
  'Pembagian dana Bryan: Rp200.000 dipegang Bryan, sisanya dipegang Maddy.'
...
where held_by_maddy.name = 'Uang Bryan'
  ...
  and not exists (
    ...
    where entry.note = 'Pembagian dana Bryan: Rp200.000 dipegang Bryan, sisanya dipegang Maddy.'
  )
  and (
    select coalesce(sum(
      case
